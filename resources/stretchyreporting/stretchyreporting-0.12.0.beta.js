...
function initialiseReporting(params) {
	...
	var reportOutputOptionsHtml = '<select id=rptOutputType><option value="HTML" selected="selected" >' 
		...
		+ doI18N("PDF Format") + '</option><option value="billingChart">'
		+doI18N("Chart Format") + '</option></select>';
...
function runTheReport()
{
	...
	switch(reportListing[reportListingIndex].type)
	{
		case "Table":
			...
			else if(showOption == "billingChart")billingChart();
		...
		case "Chart":
			reportDataSuccess = function(data, textStatus, jqXHR){
			    				createTable(data);
		    				createChart(data,reportListing[reportListingIndex].subtype);
...
function createChart(theData,rptSubType) {
	...
	RData = new google.visualization.DataTable();
 	...
 	//sarath on 3-sep-2013
		...
		case "Bar":
		    RData.addColumn('number', dataTableDef.aoColumns[1].sTitle);  
			RData.addColumn('number', dataTableDef.aoColumns[2].sTitle);  
     			break;
		case "Bubble":

			RData.addColumn('number', dataTableDef.aoColumns[1].sTitle);  
			RData.addColumn('number', dataTableDef.aoColumns[2].sTitle);  
			RData.addColumn('string', dataTableDef.aoColumns[3].sTitle); 

  				break;
				
		case "Timeline":	
			RData.addColumn({ type: 'date'}, dataTableDef.aoColumns[1].sTitle );
			RData.addColumn({ type: 'date'}, dataTableDef.aoColumns[2].sTitle );

//			RData.addColumn('date', dataTableDef.aoColumns[1].sTitle);  
//			RData.addColumn('date', dataTableDef.aoColumns[2].sTitle); 
...
/*
commented by sarath on 17-sep-2013
...
if (rptSubType=="Bubble")
	{		// 3rd column	
		if (dataTableDef.aoColumns[2].sType == "title-numeric" && dataTableDef.aaData[i][2].substr(0, 11) == '<span title') numberData = getOrigTitleNumericValue(dataTableDef.aaData[i][2])
		else numberData = dataTableDef.aaData[i][2];
		RData.setValue(ii, 2, numberData);
// 4th column 		
	RData.setValue(ii, 3, dataTableDef.aaData[i][3]);	
	};	

if (rptSubType=="Timeline")
	{ 	var dt1 = invalidDate(dataTableDef.aaData[i][1]) ;
		RData.setValue(ii, 1, new Date(dataTableDef.aaData[i][1]) );
	var dt2 = invalidDate(dataTableDef.aaData[i][2]) ;
		RData.setValue(ii, 2, new Date(dataTableDef.aaData[i][2]));
	};
			...
			case "Bar":
				...
				case "Bubble":
        		 options = {  width: 1000, 
							  ...
							  title: rptSubType,
							  hAxis: {title: 'hAxis'},
							  vAxis: {title: 'vAxis-sarath'},
							  bubble: {textStyle: {fontSize: 11}}
							};
        			Rchart = new google.visualization.BubbleChart(document.getElementById('RshowChart'));
        					Rchart.draw(RData, options);
							break;
				...
				options = 	{
          					width: 900, height: 600, title: rptSubType			
							};	
       			Rchart = new google.visualization.Timeline(document.getElementById('RshowChart'));
        					Rchart.draw(RData,options);
  				break;
			default:
  				alert("System Error: Unknown Chart Type: " + rptSubType);
