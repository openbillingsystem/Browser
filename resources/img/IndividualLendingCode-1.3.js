...
function showILClient(clientId) {
	...
	$newtabs = $("#newtabs").tabs({
	    	select: function(event, tab) {
				...
				if (tab.index == 0)
				{refreshClientBalanceInfo(clientId);
					if (clientDirty == true)
					{
...
function addILBulkMembersLoans(groupId, clientMembers){
	...
	function refreshClientBalanceInfo(clientId) {
		var successFunction =  function(data, textStatus, jqXHR) {
			$("#customerBalance").html(data.balance_amount);
			  			};
...
function popupDialogWithFormViewData(data, postUrl, submitType, titleCode, templateSelector, width, height, saveSuccessFunction)  {
	...
	if( templateSelector == '#cityFormTemplate'){
...
function repopulateOpenPopupDialogWithFormViewData(data, postUrl, submitType, titleCode, templateSelector, width, height, saveSuccessFunction){
	...
	if (templateSelector === "#oneTimeSaleFormTemplate"){
		...
		executeAjaxRequest(getUri, "POST", "", officeIdSelectSuccess,newFormData, formErrorFunction);
...
function repopulatesPlanForm() {
   ...
   var postUrl = 'paymodes';
	...
	{
			var postUrl = 'city/new';
			var getUrl = 'address/state';
			var width = 350; 
			var height = 200;
			var templateSelector = "#cityFormTemplate";
			var saveSuccessFunction = function(data, textStatus, jqXHR) {
				$("#dialog-form").dialog("close");
				
			};
			popupDialogWithFormView(getUrl, postUrl, 'GET', "Add New City", "#cityFormTemplate", width, height,saveSuccessFunction );
